...
const StockAlerts = () => {
  ...
  useEffect(() => {
    // Lọc ra các sản phẩm cần cảnh báo.
    // Lưu ý: Nếu backend không cung cấp trường maxThreshold, hãy đảm bảo các sản phẩm có trường này
    const newAlerts = products.filter(
      (product) =>
        product.quantity <= product.minThreshold ||
        (product.maxThreshold !== undefined && product.quantity >= product.maxThreshold)
    );
    setAlerts(newAlerts);
  }, [products]);
