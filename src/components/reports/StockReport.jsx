  ...
  useEffect(() => {
    const fetchStockData = async () => {
      try {
        ...
        console.log("📊 Dữ liệu tồn kho nhận được từ API:", data); // Log dữ liệu trả về
    ...
    <div className="stock-report-container">
      ...
      {loading ? (
        ...
        <div className="error-container">
          ...
          <p className="error-help">Kiểm tra kết nối đến MongoDB và đảm bảo server backend đang chạy.</p>
        </div>
      ) : stockData.length === 0 ? (
        ...
        <>
          ...
          <table className="stock-table">
            ...
            <tbody>
              {stockData.map((item) => (
                <tr key={item._id} className={item.quantity < (item.minThreshold || 10) ? "low-stock" : ""}>
                  <td>{item.code || "N/A"}</td>
                  <td>{item.name}</td>
