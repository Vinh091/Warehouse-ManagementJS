...
const OrderImport = () => {
  ...
  const handleOrderImport = async (e) => {
    e.preventDefault();
    ...
    try {
      // Gọi API tạo đơn nhập hàng (với giao dịch nhập kho)
      ...
      if (!response.ok) throw new Error('Lỗi khi tạo đơn nhập hàng');
      await response.json();
      alert('Đơn nhập hàng đã được tạo thành công!');
      setProductId('');
      setImportQuantity('');
    ...
    <div className="container mt-4">
      <div className="card">
        <div className="card-header bg-success text-white">
          <h4 className="mb-0">Tạo đơn nhập hàng</h4>
        </div>
        <div className="card-body">
          <form onSubmit={handleOrderImport}>
            {/* Nhóm chọn sản phẩm */}
            ...
            <button type="submit" className="btn btn-primary">Tạo đơn nhập hàng</button>
          </form>
        </div>
      </div>
    </div>
