...
const DeleteProduct = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setProducts(products.filter((product) => product.id !== id)); // Sử dụng product.id
    } catch (err) {
        ...
        {products.map((product) => (
          <div className="product-card" key={product.id}>
            ...
            <div className="product-details">
              ...
              <button className="delete-button" onClick={() => handleDelete(product.id)}>Xóa sản phẩm</button>
            </div>
          </div>
        ))}
      </div>
